...
static void tps43_work_handler(struct k_work *work) {
    ...
    /* Suspend may have happened after this work item was queued. */
    if (drv_data->suspended) {
        k_sem_give(&drv_data->lock);
        return;
    }
     ...
     * through TOUCH_AREA when force click is enabled, in a
     ...
     */
    uint8_t touch_data[(TPS43_REG_TOUCH_AREA + 1) - TPS43_REG_GESTURE_EVENTS_0];
    size_t read_len = config->force_click ? sizeof(touch_data)
                                         : (TPS43_REG_REL_Y + 2) - TPS43_REG_GESTURE_EVENTS_0;
    ...
    if (config->force_click && (touch_data[2] & TPS43_SHOW_RESET)) {
        ...
        if (ret < 0) {
            LOG_ERR("Force click sensor reconfiguration failed: %d", ret);
        }
        goto done;
    }
    ...
    if (config->force_click) {
        /* Event mode does not notify strength-only changes. Stream while a
         * finger is present, then restore event mode as soon as it leaves. */
        if (is_touching != drv_data->force_streaming) {
            uint8_t events = is_touching ? drv_data->event_config & ~TPS43_EVENT_MODE
                                         : drv_data->event_config;
            ret = tps43_i2c_write_reg8(dev, TPS43_REG_SYSTEM_CONFIG_1, events);
            if (ret < 0) {
                tps43_force_cancel_and_report(dev);
                goto done;
            }
    ...
    if (!config->force_click && !(gestures_events[0] & TPS43_PRESS_AND_HOLD) && is_drag_active) {
